  ...
  l1.setBounds(50,100,250,25);
	...
	if(ae.getSource()==submit)
     ...
     try
     {
        ...
        rs = stmt.executeQuery("SELECT sid,sname,mbno,mname,mexpdate,mqty,mpurprice,msaleprice from fish where mpurdate='"+t1.getText()+"' ");
          while(rs.next())
            {
              ...
              if (foundrec == 0)
                {
                    JOptionPane.showMessageDialog(null,"Not any Fish purchase on given date","Dialog",JOptionPane.WARNING_MESSAGE);
                }
	        }
             con.close();
       }
